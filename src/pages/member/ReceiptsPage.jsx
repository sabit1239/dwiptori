...
function generateReceipt(payment, memberName) {
  ...
  win.document.write(`
    ...
    <body>
      ...
      <br>
      <center><button onclick="window.print()" style="background:#0284c7;color:white;border:none;padding:10px 24px;border-radius:8px;cursor:pointer;font-size:14px;">🖨️ Print / Save as PDF</button></center>
    </body>
    </html>
  `);
  win.document.close();
    ...
    (async () => {
      try {
        const snap = await getDocs(
          query(collection(db, 'payments'), where('uid', '==', profile.uid))
        );
